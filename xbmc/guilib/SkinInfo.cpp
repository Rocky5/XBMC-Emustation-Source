...
{
  if (res.Equals("xml_sd_pal"))
    return PAL_4x3;
  else if (res.Equals("xml_sd_pal"))
    return PAL_16x9;
  else if (res.Equals("xml_sd_ntsc"))
    return NTSC_4x3;
  else if (res.Equals("xml_sd_ntsc"))
    return NTSC_16x9;
...
bool CSkinInfo::GetResolution(const TiXmlNode *root, const char *tag, RESOLUTION &res) const
{
  ...
  {
    strRes.ToLower();
    if (strRes == "xml_sd_pal")
      res = PAL_4x3;
    else if (strRes == "xml_sd_pal")
      res = PAL_16x9;
    else if (strRes == "xml_sd_ntsc")
      res = NTSC_4x3;
    else if (strRes == "xml_sd_ntsc")
      res = NTSC_16x9;
    else if (strRes == "xml")
