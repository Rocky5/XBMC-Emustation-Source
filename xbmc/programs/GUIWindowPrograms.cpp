...
bool CGUIWindowPrograms::GetDirectory(const CStdString &strDirectory, CFileItemList &items)
{
	...
	int fastgameparsing = g_guiSettings.GetBool("mygames.gamesfasterparsing");
	...
	for (int i = 0; i < items.Size(); i++)
	{
		...
		if (item->IsXBE())
		{
			CStdString XBE_path = item->GetPath();

			if (URIUtils::GetFileName(XBE_path).Equals("default_ffp.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}
			else if (URIUtils::GetFileName(XBE_path).Equals("default_p.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}    
			else if (URIUtils::GetFileName(XBE_path).Equals("defaultws.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}   
			else if (URIUtils::GetFileName(XBE_path).Equals("defaultxhd.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}   
			else if (URIUtils::GetFileName(XBE_path).Equals("default720p.xbe"))
			{
				m_vecItems->Remove(i--);
				continue;
			}
			...
			if (!dwTitleID)
			{
				...
				if (CUtil::GetXBEDescription(XBE_path, description) && (!item->IsLabelPreformated() && !item->GetLabel().IsEmpty()))
				{
					// Get synopsis title for description if found and the settings is enabled
					if (synopsisparsing)
					{
						URIUtils::AddFileToFolder(XBE_path,"_resources\\default.xml",XBE_path);
						if (CFile::Exists(XBE_path))
						{
							TiXmlDocument xml_path_load;
							xml_path_load.LoadFile(XBE_path);
				...
				dwTitleID = CUtil::GetXbeID(XBE_path);
				if (!item->IsOnDVD())
			...
			if (g_guiSettings.GetBool("mygames.gamesynopsisinfo"))
			{
				// item->SetLabelSynopsis_Developer(m_database.GetSynopsisDeveloper(XBE_path));
				// item->SetLabelSynopsis_Publisher(m_database.GetSynopsisPublisher(XBE_path));
				// item->SetLabelSynopsis_FeaturesGeneral(m_database.GetSynopsisFeaturesGeneral(XBE_path));
				// item->SetLabelSynopsis_FeaturesOnline(m_database.GetSynopsisFeaturesOnline(XBE_path));
				// item->SetLabelSynopsis_ESRB(m_database.GetSynopsisESRB(XBE_path));
				// item->SetLabelSynopsis_ESRBDescriptors(m_database.GetSynopsisESRBDescriptors(XBE_path));
				// item->SetLabelSynopsis_Genre(m_database.GetSynopsisGenre(XBE_path));
				// item->SetLabelSynopsis_ReleaseDate(m_database.GetSynopsisReleaseDate(XBE_path));
				// item->SetLabelSynopsis_Rating(m_database.GetSynopsisRating(XBE_path));
				// item->SetLabelSynopsis_Platform(m_database.GetSynopsisPlatform(XBE_path));
				// item->SetLabelSynopsis_Exclusive(m_database.GetSynopsisExclusive(XBE_path));
				item->SetLabelSynopsis_Overview(m_database.GetSynopsisOverview(XBE_path));
			}
